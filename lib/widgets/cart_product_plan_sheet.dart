...
class CartProductPlanSheet extends StatelessWidget {
  const CartProductPlanSheet({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    // 🔥 PREFILL WHEN PLAN ALREADY APPLIED
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (controller.isCartPlanApplied.value) {
        if (controller.customDays.value > 0) {
          dayController.text = controller.customDays.value.toString();
        }

        if (controller.customAmount.value > 0) {
          amountController.text =
              controller.customAmount.value.toStringAsFixed(0);
        }
      }
    });

    dayController.addListener(() {
      controller.updateCartAmountFromDays(dayController, amountController);
    });

    amountController.addListener(() {
      controller.updateCartDaysFromAmount(dayController, amountController);
    });
                ...
                children: [
                  ...
                  GestureDetector(
                    onTap: () {
                      ...
                      controller.applyCartPlan(amount);
                      Get.back();
                    },
                    child: Container(
              ...
              Obx(() {
                ...
                return appButton(
                  onTap: () {
                    ...
                    final double perDay = selected["amount"];
                    ...
                    controller.applyCartPlan(perDay);
                    Get.back();
                  },
